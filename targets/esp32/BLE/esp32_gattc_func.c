...
void gattc_evt_search_res(esp_gatt_if_t gattc_if,esp_ble_gattc_cb_param_t *param){
	...
	if (bleuuid_uuid_equal(srvc_id->id.uuid, serviceFilter)) {
		get_server = true;
...
uint32_t gattc_disconnect(){
	return esp_ble_gattc_close(gattc_apps[GATTC_PROFILE].gattc_if,gattc_apps[GATTC_PROFILE].conn_id);
}
